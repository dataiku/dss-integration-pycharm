...
public class VirtualFileUtils {
    @NotNull
    public static VirtualFile createFile(final Object requestor, final VirtualFile parent, final String name) throws IOException {
    ...
    @NotNull
    public static VirtualFile getOrCreateDirectory(Object requestor, VirtualFile parent, String name) throws IOException {
        VirtualFile projectFolder = parent.findChild(name);
        if (projectFolder == null) {
            projectFolder = createDirectory(requestor, parent, name);
    ...
    @NotNull
    public static VirtualFile createDirectory(final Object requestor, final VirtualFile parent, final String name) throws IOException {
        Application application = ApplicationManager.getApplication();
        if (application.isWriteAccessAllowed()) {
    ...
    public static void writeToFile(final VirtualFile file, final String recipeContent) throws IOException {
        Application application = ApplicationManager.getApplication();
        if (application.isWriteAccessAllowed()) {
            file.setBinaryContent(recipeContent.getBytes(UTF_8));
            file.setCharset(UTF_8);
        } else {
    ...
    public static VirtualFile getOrCreateVirtualFile(Object requestor, VirtualFile parent, String... names) throws IOException {
        if (names.length == 0) {
            ...
            return createFile(requestor, parent, names[0]);
        } else {
            VirtualFile directory = getOrCreateDirectory(requestor, parent, names[0]);
